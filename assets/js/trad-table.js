...
(function ($) {
  ...
  function initTable(wrap) {
    ...
    var table      = wrap.querySelector('.trad-table');
    var searchInput = wrap.querySelector('.trad-table-search');
    if (!table) return;

    var isSortable = table.classList.contains('trad-table--sortable');
      ...
      headers.forEach(function (th, colIdx) {
        th.addEventListener('click', function () {
          ...
          headers.forEach(function (h) {
            h.classList.remove('trad-sort-asc', 'trad-sort-desc');
          });
          th.classList.add('trad-sort-' + dir);
    ...
    // ── Pagination ───────────────────────────────────────────────────────────
    var paginationWrap = wrap.querySelector('.trad-table-pagination');
    ...
    function getVisibleRows() {
      return Array.prototype.slice.call(tbody.querySelectorAll('tr:not(.trad-hidden)'));
    }
    ...
    function renderPagination() {
      ...
      function makePage(label, page, disabled, active) {
        ...
        btn.className = 'trad-table-page-btn' + (active ? ' trad-active' : '') + (disabled ? ' trad-disabled' : '');
    ...
    if (searchInput && !enablePagination) {
      searchInput.addEventListener('input', function () {
        ...
        rows.forEach(function (row) {
          ...
          row.classList.toggle('trad-hidden', q !== '' && text.indexOf(q) === -1);
        });
      });
    }
  }
  ...
  $(window).on('elementor/frontend/init', function () {
    elementorFrontend.hooks.addAction('frontend/element_ready/trad-table.default', function ($scope) {
      initTable($scope[0].querySelector('.trad-table-wrap'));
    });
  });

  document.addEventListener('DOMContentLoaded', function () {
    document.querySelectorAll('.trad-table-wrap').forEach(initTable);
  });
